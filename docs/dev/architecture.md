...
- En el admin, drop-zones `__gap__` (DOM-only, no se persisten) entre grupos reales y al inicio/fin. Al soltar una card se crea o reusa un ghost. Cada lista Sortable tiene un filler in-flow (no vacío) y `draggable: '.card-item'`. Las instancias se reconcilian con `Sortable.get` (no destroy+recreate en cada drop) y un guard en `lib/sortable-guard.ts` evita `_onDragOver` con `el` null tras destroy.
